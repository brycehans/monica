  ...
  <div>
    ...
    >
      <template #table-row="slotProps">
        <span v-if="slotProps.column.field === 'message_count'">
          <span>
            {{ slotProps.row.message_count }}
...
const columns = [
  { label: t('app.date'), field: 'happened_at', tdClass: 'vgt-table-date' },
  { label: t('app.type'), field: 'contact_field_type', width: '110px' },
  { label: t('people.conversation_list_table_messages'), field: 'message_count', width: '110px' },
  { label: t('people.conversation_list_table_content'), field: 'content' },
];

onMounted(async () => {
  const response = await axios.get('people/' + props.hash + '/conversations');
  conversations.value = response.data as Conversation[];
});

function onRowClick(params: { row: Conversation }) {
